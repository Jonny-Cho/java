 ...
 * Java BingoGame
 * 
 * @author Youzang MTGi 사원 유창근 자바코딩 실력 쌓을겸 만들어봤어요 걍 넣고 돌리시면 되요
 */
public class BingoEx extends Frame {
	...
	private String[] values = { "송성식", "김형선", "이주엽", "강용운", "이대엽", "김지훈", "문승도", "서종현", "김한주", "홍정우", "주한나", "이성민",
			"유창근", "고병욱", "변윤서", "이정현", "전지현", "송혜교", "아즘마", "김사랑", "최윤호", "최병걸", "문승명", "장천호", "임재환" };
	...
	boolean checkBingo() {
		...
		for (int i = 0; i < bArr.length; i++) {
			for (int j = 0; j < bArr[i].length; j++) {// 가로검사
				if (bArr[i][j] == true) {
					garoCnt++;
					if (garoCnt == SIZE) {// 2차 배열을 1차 배열 형식으로 만들어야 한다 begin과
											// end를 이용하여 만든다
						int begin = i * SIZE;
						int end = begin + SIZE;
				...
				if (bArr[j][i] == true) {// 세로검사
					seroCnt++;
					if (seroCnt == SIZE) {
						for (int n = i; n < btnArr.length; n = n + 5) {// 5식증가하여
																		// 색상 변경
							btnArr[n].setBackground(Color.green);
						}
					}
				...
				if (i == j && bArr[i][j] == true) {// 대각선검사
					crossCnt1++;
					if (crossCnt1 == SIZE) {
						for (int n = 0; n < btnArr.length; n = n + 6) {// 대각선으로
																		// 0부터
																		// 24까지
																		// 6식증가하여
																		// 색상변경
				...
				if (((bArr.length - 1) - i) == j && bArr[i][j] == true) {// 역대각선
																			// 검사
					corssCnt2++;
					if (corssCnt2 == SIZE) {
						/*
						 ...
						 */
						for (int n = (btnArr.length - 4); n > 0; n--) {// 포문을
																		// 역으로
																		// 돌려서
																		// 검사
							if (n % 4 == 0)
		...
		if (crossCnt1 == SIZE)
			bingoCnt++;// 대각선과 역대각선은 1번식만 검사해주면된다
		if (corssCnt2 == SIZE)
			bingoCnt++;
	...
	class MyEventHandler extends WindowAdapter implements ActionListener {
		@Override
		public void actionPerformed(ActionEvent ae) {// 버튼 클릭스 actionPerformed
														// 메서드 오버라이딩 하여 Event
														// 핸들링
			...
			for (int i = 0; i < btnArr.length; i++) {// 버튼배열과 현제 눌려진 버튼 이벤트의 객체가
														// 같은게 있다면..
				if (btnArr[i] == btn) {
